...
- 状态：`[x]` 已完成并通过 Reviewer 验收
- 完成日期：2026-08-10
- EA-021A：Application 新增通用 `IAsyncQueryExecutor`，Infrastructure 使用 EF Core 异步扩展实现并完成 DI 注册；Application 未引入 EF Core 依赖。目标查询统一传递 `CancellationToken`，测试环境提供可记录执行次数和物化行数的内存实现。
- EA-021B：用户列表改为计数、当前页投影和当前页角色关联 3 次固定查询，移除逐用户加载角色 ID/编码的 N+1，并排除密码哈希等无关字段；角色列表、角色成员、权限矩阵、关系清理、存在性校验和授权失效涉及的数据库读取改为异步。角色成员使用数据库分页和部门左连接，保留内置角色、超级管理员及最后一个超级管理员保护规则。
- EA-021C：待办、已办和抄送改为任务/抄送与工作流实例数据库关联后执行关键字过滤、计数、排序和分页；不再全量加载后内存过滤或分页。我的发起列表使用轻量投影，不读取 `FormDataJson`，实例详情和审批记录继续返回完整数据并改为异步读取。
- EA-021D：报表定义列表改为轻量 DTO 投影，不再读取 `ColumnsJson`、`ParamsJson` 和参数集合，消除逐报表参数查询；编辑页改为打开时按 ID 加载完整详情，报表查看页继续按选择项加载详情。报表参数同步、删除、执行前参数读取和执行日志分页均改为异步。
- EA-021E：操作日志、登录日志、Outbox、Inbox 使用异步计数、数据库分页和 DTO 投影；Outbox 列表不读取 `Payload`、`Headers`，详情接口继续完整返回。Inbox 幂等判断和消息定位改为异步。API 调用日志只批量加载当前页涉及的 Client，不再预加载全部 Client。
- 兼容策略：所有现有路由、请求 DTO 和响应类型保持不变；报表列表的详情字段与 Outbox 列表的 `Headers` 改为空值，现有前端已切换为详情接口按需加载。详情接口、租户过滤、权限策略和审计行为不变。
- 数据库变更：无。未新增实体、字段、索引或 EF Migration；`dotnet ef migrations has-pending-model-changes` 确认模型无待处理变更。现有性能索引已覆盖本次默认筛选和排序路径，本项未在缺少真实执行计划证据时新增索引。
- 性能基线：新增 5 项 EA-021 专项测试固定查询次数和物化上界。10 行用户页由原先 22 次查询降为 3 次；100 条工作流待办仍为 2 次查询，但由全量物化 100 条任务和 100 条实例降为只物化当前页 10 条关联结果；10 行报表页由 12 次查询降为 2 次；10 行 Outbox 页保持 2 次查询但不再物化 122,880 个大字段字符；10 行 API 调用日志页保持 3 次查询，但 Client 物化由 50 条降为当前页 10 条。详细记录见 `docs/ea-021-query-performance-baseline.md`。
- 验证结果：EA-021 专项测试 5 项通过，单项 InMemory 耗时 9–46 ms；`PermissionSystem.UnitTests` 145 项通过，`PermissionSystem.Tests` 43 项通过；`PermissionSystem.IntegrationTests` 11 项通过，16 项依赖真实 SQL Server 的既有用例按环境条件跳过。后端解决方案 Release 构建 0 错误；前端 `vue-tsc -b && vite build` 通过；`git diff --check` 通过。
- Reviewer 结论：通过。A～E 子批次均保持 Application/Infrastructure 依赖边界，目标列表已使用异步执行、数据库分页和 DTO 投影；已识别的用户/报表 N+1、工作流内存分页及 API Client 全量加载已闭环，租户与授权规则未被绕过。
- 剩余风险：本机没有连接真实 SQL Server，尚未记录生产数据规模下的实际执行计划、网络往返耗时、数据库内存授予和逻辑读；专项耗时仅作为 InMemory 代码路径回归基线。`Contains` 关键字查询仍受数据库排序规则和 `%keyword%` 索引能力限制，大规模模糊搜索需基于真实执行计划单独治理。报表列表和 Outbox 列表的详情字段改为按需加载，仓库外调用方若依赖列表直接携带这些字段需同步改用详情接口。既有 NuGet 漏洞警告、ASP.NET 测试 API 过时警告及前端大 chunk 警告不属于本项，尚未处理。
