...
GO
    ...
    WHERE YEAR(a.F_JN_CUSTOMERDELIVERDATE) = @Year 
      AND MONTH(a.F_JN_CUSTOMERDELIVERDATE) = @Month;
...
DECLARE @case_when varchar(max) = N'';
DECLARE @fdatavalue NVARCHAR(100);
DECLARE @series_cond varchar(max); -- 读取的系列条件
DECLARE @process_cond varchar(max); -- 读取的工艺条件
DECLARE @ys_cond varchar(max); -- 读取的颜色条件
DECLARE @cp_cond varchar(max); -- 读取的半成品条件
...
BEGIN
    ...
    SET @current_when = @current_when + N' THEN N''' + @fdatavalue + N''' ' + CHAR(13);
...
---获取产能
   SELECT 
        单据类型 = c.FNAME,
        系列大类 = d.FDATAVALUE,
        产能 = a.FSTANDARDCAPACITY 
    INTO #tempbjcnts
    FROM JN_t_WorkCapacityEntry a
    LEFT JOIN JN_t_WorkCapacity b ON a.fid = b.fid
