...
| 项目 | 内容 |
| --- | --- |
| 文档状态 | P0 至 P3、P5-A 已实现；P4 批次 1 已实现，P4-B2 与生产环境验收待确认 |
| 版本 | v1.2 |
| 日期 | 2026-08-31 |
...
- MCP 请求限流、IP 白名单、外部调用日志、脱敏、数据集失败熔断和管理员告警。
...
| 安全裁剪 | 动态客户端注册和交互式用户授权继续关闭；当前只开放预配置 Client Credentials/Bearer Token。 |
| 未完成 | MCP 协议与真实第三方客户端的生产兼容矩阵、生产等价 OAuth 联调和独立扩容压测。 |
...
| 运行保护 | Redis 分布式限流、取消、数据集处理器熔断、管理员通知、稳定错误映射和独立健康检查 |
| 审计 | 记录客户端、租户、Tool、数据集、输入摘要、行数、耗时、状态和 TraceId |
| OAuth 发现 | `/.well-known/oauth-protected-resource`、`/.well-known/oauth-protected-resource/mcp` 和 401 `resource_metadata` Challenge |
| Schema 治理 | 稳定 Hash、发布状态、授权快照；Schema 变化后旧授权自动失效并由管理员重新确认 |
...
4. **协议接入标准化（代码基线已实现，环境验收待完成）**
   - 明确支持的第三方智能体及版本，建立 `initialize`、`tools/list`、`tools/call`、错误映射、超时和大结果兼容矩阵。
   - 已按 RFC 9728 提供 Protected Resource Metadata 和 401 Challenge；目标客户端仍需执行生产等价环境兼容性验证。
   - 动态客户端注册和交互式用户授权继续关闭；对外接入文档必须明确 MCP URL、Token URL、Scope 和 Client Credentials 配置方式，Secret 只允许一次回显并通过安全渠道交付。

5. **测试与运营（自动化基线已实现，生产验收待完成）**
...
| 子阶段 | 范围 | 数据库影响 | 退出条件 |
| --- | --- | --- | --- |
| P4-B1（已实现） | 抽取处理器接口和注册表，将现有两个数据集迁移到处理器 | 无，复用现有表 | 现有契约和授权行为不变，目标单元测试通过 |
| P4-B2（待业务确认） | 接入第一个 ERP/WMS 等真实业务只读数据集 | 原则上无；若业务范围无法由现有授权表达，先提交 DBA 方案 | 业务口径、Owner、字段分级、租户和服务数据范围均已确认 |
| P4-B3（基础已实现） | Schema 版本/Hash、发布状态和授权变更治理；通用人工审批发布工作流按实际需要后续建设 | 增加 Schema/发布/授权快照字段及索引，迁移兼容回填 | 新字段不会自动暴露，Schema 变化使旧授权失效；人工审批流尚未建设 |
| P4-B4（代码基线已实现） | OAuth 自动发现、契约/压测脚本、处理器熔断和告警；真实客户端矩阵仍需环境验收 | 无新增业务表；复用现有通知和熔断状态 | 目标智能体完成生产等价环境端到端验收后关闭本阶段 |
...
- MCP 契约测试：使用 `scripts/mcp-contract-test.ps1` 验证元数据、401 Challenge、初始化、工具发现、调用和 Schema。
- 数据安全测试：跨租户、数据范围、敏感字段、越权和提示词注入。
- 业务测试：DemoBusinessOrder 草稿、并发确认、幂等创建和工作流关联。
- 前端测试：聊天弹窗、会话切换、流式/分段响应、失败重试和权限隐藏。
- 性能测试：模型调用 P95、Tool 查询 P95、并发 Run、日志写入和 MCP 扩容；MCP 基线使用 `scripts/mcp-load-test.ps1`。
...
`P0 → P1 → P2 → P3`、P4 批次 1 和 P5-A 已在仓库中落地。下一步应先确认 P4-B2 的业务 Owner、查询口径和服务数据范围，再接入真实 ERP/WMS 只读数据集；同时用契约与负载脚本完成目标外部智能体的生产等价环境联调。只有确需非代码化发布职责分离时再建设 P4-B3 通用人工审批流；外部 MCP 在完整 OAuth、租户隔离、限流、压测和告警验收前不得直接开放到不受控网络。
